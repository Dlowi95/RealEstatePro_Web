...
import { Box, Heading, Badge, Button, Avatar, Spinner, Table } from '@chakra-ui/react';
...
export default function AdminProperties() {
  ...
  useEffect(() => { if (isAdmin) fetchPendingProps(); }, [isAdmin]);
  ...
  return (
    <Box>
      <Heading size="lg" mb={6}>⏳ Duyệt tin đăng ({properties.length})</Heading>
      {properties.length === 0 ? (
            ...
            <Table.Body>
              {properties.map(prop => (
                <Table.Row key={prop._id}>
                  ...
                  <Table.Cell><Badge colorScheme={prop.type === 'Buy' ? 'green' : 'blue'}>{prop.type === 'Buy' ? 'Mua' : 'Thuê'}</Badge></Table.Cell>
                  <Table.Cell>{prop.price.toLocaleString()} VNĐ</Table.Cell>
                  <Table.Cell>{prop.area} m²</Table.Cell>
                  <Table.Cell><Avatar size="xs" name={prop.userId?.fullName} /> {prop.userId?.fullName}</Table.Cell>
                  <Table.Cell>
                    <Button size="sm" leftIcon={<FaCheck />} colorScheme="green" onClick={() => handleApprove(prop._id)} mr={2}>Duyệt</Button>
                    <Button size="sm" leftIcon={<FaEyeSlash />} colorScheme="red" onClick={() => handleToggleHide(prop._id)}>Ẩn</Button>
                  </Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
